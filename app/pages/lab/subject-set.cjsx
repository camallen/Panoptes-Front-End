    ...
    <div>
      ...
      <p>
        <UploadDropTarget accept="text/csv, text/tab-separated-values, image/*" multiple onSelect={@handleFileSelection}>
          ...
          Manifests must be <code>.csv</code> or <code>.tsv</code>. The first row should define metadata headers.<br />
          All other rows should include at least one reference to an image filename in the same directory as the manifest.<br />
          If using a manifest file you must select the files that you want to upload as well.<br />
